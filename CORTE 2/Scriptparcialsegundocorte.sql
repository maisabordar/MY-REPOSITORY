-- PARCIAL SEGUNDO CORTE / INGENIERIA DE DATOS
...
-- PREGUNTA No. 15
/* Cree el procedimiento sp_informe_especialidad(p_especialidad VARCHAR(80)) que: 
a) Valide que exista al menos un medico activo con esa especialidad (si no, error). 
(b) Retorne con un SELECT el resumen de esa especialidad: total_medicos_activos, 
	promedio_salario, total_consultas_realizadas, ingreso_total_generado, paciente_mas_frecuente 
	(nombre del paciente con mas consultas en esa especialidad, usando subconsulta). 
(c) Adicionalmente liste con otro SELECT los nombres de los pacientes que han tenido consultas
	con esa especialidad pero que TAMBIEN hayan tenido consultas canceladas en cualquier 
    especialidad (use interseccion con subconsultas IN).
Clausulas requeridas: CREATE PROCEDURE, multiples SELECT con JOIN, subconsulta escalar para paciente_mas_frecuente, subconsulta con IN doble
*/
...
DELIMITER //
create procedure sp_informe_especialidad(
		IN p_especialidad VARCHAR(80)
        OUT p_mesanje VARCHAR(60)
) 
BEGIN
        ...
        BEGIN
			rollback;
            set p_mensaje='NO EXISTE NINGUN MEDICO ESPECIALISTA ACTIVO EN ESTA ESPECIALIDAD';
		end;
	
    
        -- resumen de la especialidad
        select total_medicos_activos, promedio_salario, total_consultas_realizadas, ingreso_total_generado, paciente_mas_frecuente
        from (  SELECT medico_id,
                AVG(salario) as promedio_salario,
                sum(
                sum(costo) as ingreso_total_generado,
                as paciente_mas_frecuente);
                
        
        -- pacientes que hayan tenido consulta con esta especialidad y otra que le hayan cancelado
       select 
			p.paciente,
			m.especialidad,
			p.estado,
		from citas_canceladas as cc
		inner join pacientes p on cc.nombreFK = p.nombre
		inner join medicos m on cc.especialidad = m.especialidad
		inner join consultas c on cc.estadpFK = c.estado;
       
       select p.nombre, m.especialidad
	   from cc
        where (
			select estado
            from estado in ("cancelado"));
